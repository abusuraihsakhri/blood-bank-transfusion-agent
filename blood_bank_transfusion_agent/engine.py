"""
Clinical Algorithmic Engine & Guideline Rules for TransfusionGuard: Autonomous Blood Bank Compatibility & Crossmatch Safety Agent.
Domain: Transfusion Medicine
Standard: AABB Technical Manual 20th Edition
"""
import math
...
class ClinicalDomainEngine:
    GUIDELINE = "AABB Technical Manual 20th Edition"
    ...
    def evaluate_primary_index(cls, value: float) -> Optional[Dict[str, Any]]:
        if value > cls.PRIMARY_BASELINE_LIMIT:
            return {
                ...
                "finding": f"Observed value ({value:.2f}) exceeds AABB Technical Manual 20th Edition clinical baseline limit ({cls.PRIMARY_BASELINE_LIMIT:.1f}).",
    ...
    def evaluate_biomarker_concordance(cls, status_flag: str, biomarkers: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if "DISCORDANT" in status_upper or "EQUIVOCAL" in status_upper or "MUTANT" in status_upper:
            return {
                ...
                "recommendation": f"Order reflex confirmatory testing per AABB Technical Manual 20th Edition clinical recommendations.",
            }
        return None
